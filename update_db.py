...
# get courses from database, remove existing courses from courses
course_names = {
    course: get_readable_course_name(course) for course in courses
}
# reload courses from db to memory
...
logger.info(f'Fetching events for {len(courses)}...')
course_events = {}
for i, course in enumerate(course_names):
    logger.info(f'Fetching event {i}/{len(course_names)}')
    course_events[course] = get_all_sanctioned_events(course, after_date)

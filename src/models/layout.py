...
class AggregateLayout:
    ...
    def get_averaged_distances(self) -> list[int]:
        ...
        for i in range(self.num_holes):
            averaged_distance = int(
                np.mean([int_distances[j][i] for j in range(self.num_layouts)])
            )
...
def aggregate_layouts(
    rounds: list[Round], threshold: int = 0.5
    ...
    clustered_rounds, ranges = cluster_rounds(rounds, 0)
